...
export interface SubmittedTicketsItem {
  ticketId: number;
  oddsTotal: number;
  betAmount: number;
...
export class SubmittedTicketsDataSource extends DataSource<SubmittedTicketsItem> {
   ...
   */
  connect(): Observable<SubmittedTicketsItem[]> {
   ...
   */
  private getPagedData(data: SubmittedTicketsItem[]): SubmittedTicketsItem[] {
    if (this.paginator) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: SubmittedTicketsItem[]): SubmittedTicketsItem[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this.sort?.active) {
        case 'name': return compare(a.ticketId, b.ticketId, isAsc);
        case 'id': return compare(+a.betAmount, +b.betAmount, isAsc);
        default: return 0;
      }
    });
  }
